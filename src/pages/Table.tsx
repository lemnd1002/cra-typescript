...
const Table: React.FunctionComponent<{}> = props => {
    ...
    useEffect(() => {
        async function onFetch() {
            try {
                const res = await baseURL.get(`/employees?page=${pageNumber}&limit=${pageSize}&sortBy=createdAt&order=desc`);
                setList(res.data);
              } catch {
    ...
    const handleSubmit = async (e: any) => {
        e.preventDefault();
        if(!input.name || !input.email || !input.position) return
    ...
    return (
        <Container className="text-center table-employees">            
            ...
            <div>
                <Button disabled={pageNumber < 2} className="custom-button" color="primary" onClick={handlePrevious}>Previous</Button>
                <Button className="custom-button" color="primary" onClick={handleNext}>Next</Button>
            </div>
            <List people={list}/>
        </Container>
    );
